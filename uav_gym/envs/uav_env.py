...
import random
import datetime
# from CKM.mixed_CKM import predict_with_noise
...
class uavEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # if uavEnv.model is None:
        #     print('load model')
            # self.model = tf.keras.models.load_model('../CKM/uav_user_channel_model1.h5')
        # self.model = uavEnv.model
        # self.model = tf.keras.models.load_model('../CKM/uav_user_channel_model2.h5')
        self.model = load_model(r'D:\A_codes\Trans2024\CKM\kdml_model_with_knowledge_in_hidden_layer_augmented.h5')
        print('model loaded')
        ...
        # 定义无人机与哪个用户通信和用户通信功率
        self.choose_iotd_num = np.ones(self.iotd_num, dtype=np.int64) * 100
        self.choose_iotd_power = 0.0
        # 定义无人机与所有用户的距离
        self.uav_iotd_horizontal_distance = np.zeros(self.iotd_num)
        self.uav_iotd_horizontal_distance_err = np.zeros(self.iotd_num)
        self.uav_iotd_real_distance_err = np.zeros(self.iotd_num)
        self.uav_iotd_real_distance = np.zeros(self.iotd_num)
        self.uav_iotd_theta = np.zeros(self.iotd_num)
        self.uav_iotd_theta_err = np.zeros(self.iotd_num)
        self.uav_iotd_theta_temp = np.zeros(self.iotd_num)
        self.uav_iotd_path_loss = np.zeros(self.iotd_num)
        self.uav_iotd_path_loss_err = np.zeros(self.iotd_num)
        self.iotd_receive_power = np.zeros(self.iotd_num)
        self.iotd_received_power_pred_n = np.zeros(self.iotd_num)
        self.iotd_receive_power_err = np.zeros(self.iotd_num)
        # 归一化参数(各项标准的最大值)
        ...
        self.step_cnt = 0
        self.env = np.load('../CKM/environment_scaled.npy')
        self.env_now = self.env[0:self.iotd_num]
    ...
    def step(self, action):
        ...
        self._calculate_disance_and_theta()
        # 7、选择用户功率
        self.inputs, self.inputs_err = self._inputs()
        ...
        self._calculate_all_iotd_receive_power()
    ...
    def _reset_iotd_pos(self):
        ...
        self.x = np.zeros(self.iotd_num)
        self.y = np.zeros(self.iotd_num)
        self.z = np.zeros(self.iotd_num)
        self.iotd_pos = []
        self.xyz = []
        # for i in range(self.iotd_num):
        #     self.iotd_x_pos[i] = np.random.randint(0,1000,1)
        #     self.iotd_y_pos[i] = np.random.randint(0, 1000, 1)
        #     self.iotd_z_pos[i] = np.random.randint(0, 250, 1)
        #     self.iotd_pos.append([self.iotd_x_pos[i], self.iotd_y_pos[i], self.iotd_z_pos[i]])
        with open('../CKM/UE_q.txt') as f:
            for i in range(self.iotd_num):
                l = f.readline().strip('\n').split(' ')
                self.iotd_x_pos[i] = eval(l[0])
                self.iotd_y_pos[i] = eval(l[1])
                self.iotd_z_pos[i] = eval(l[2])
                self.iotd_pos.append([self.iotd_x_pos[i], self.iotd_y_pos[i], self.iotd_z_pos[i]])
            iotd_pos_path = 'train\pos'
            if not os.path.exists(iotd_pos_path):
                os.makedirs(iotd_pos_path)

            if self.ep == self.uavConfig.train_time:
    ...
    def _reset_record_all_eposide(self):
        ...
        self.all_ep_endt = []
        self.pre_reward = 0
        self.cnt_convergence = 0
        self.flag_convergence = False
    ...
    def _reset_record_one_eposide(self):
        ...
        #  用来保存无人机
        self.save_uav_track = []
        self.save_uav_track.append([0,0,250])
    ...
    def _calculate_per_iotd_loss(self):
        loss = 0
        for i in range(self.iotd_num):
            if self.finish_com_task_flag[i] == 1:
                loss += 0.
            else:
    ...
    def _calculate_iotd_per_R(self):
        for i in range(self.iotd_num):
            if self.is_com[i] == 1:
                ...
                self.R_remain[i] += r_temp
                if self.R_remain[i] >= self.R_min:
                    self.finish_com_task_flag[i] = 1
                    self.payload[(self.step_cnt - 1):, i] = 0.0
                    # self.payload *= 0
                else:
                    self.finish_com_task_flag[i] = 0
                    self.payload[(self.step_cnt - 1):, i] = self.R_min - self.R_remain[i]
    ...
    def _calculate_disance_and_theta(self):
        ...
        for i in range(self.iotd_num):
            ...
            self.x[i] = self.iotd_x_pos[i]-np.random.normal(0,sigma,1)
            self.y[i] = self.iotd_y_pos[i]-np.random.normal(0,sigma,1)
            self.z[i] = self.iotd_z_pos[i]-np.random.normal(0,sigma,1)
            self.xyz.append([self.x[i],self.y[i],self.z[i]])

            self.uav_iotd_horizontal_distance_err[i] = np.sqrt(np.square(self.uav_x_pos - self.x[i])
                                                           + np.square(self.uav_y_pos - self.y[i])) + 1
            ...
            self.uav_iotd_real_distance_err[i] = np.sqrt(np.square(self.uav_iotd_horizontal_distance_err[i])
                                                         + np.square(self.uav_z_pos - self.z[i]))

            self.uav_iotd_theta[i] = np.arctan(
                np.true_divide(self.uav_z_pos - self.iotd_z_pos[i], self.uav_iotd_horizontal_distance[i]))
            self.uav_iotd_theta_err[i] = np.arctan(
                np.true_divide(self.uav_z_pos - self.z[i], self.uav_iotd_horizontal_distance_err[i]))

            self.uav_iotd_path_loss[i] = np.true_divide(self.A, 1 + self.a * np.exp(
                -self.b * (180 / np.pi * self.uav_iotd_theta[i] - self.a))) + 20 * np.log10(
                self.uav_iotd_real_distance[i]) + self.B

            self.uav_iotd_path_loss_err[i] = np.true_divide(self.A, 1 + self.a * np.exp(
                -self.b * (180 / np.pi * self.uav_iotd_theta_err[i] - self.a))) + 20 * np.log10(
                self.uav_iotd_real_distance_err[i]) + self.B
    ...
    def _update_uav_state(self):
        ...
        self.uav_state_normalization[5] = self.choose_iotd_power / self.p_max
        for i in range(self.iotd_num):
            self.VEC = 6
    ...
    def _choose_uav_dir(self, action, uav_theta):
        if not self.uav_finish_com_task_flag:
            ...
            choose_uav_theta = action_1_temp * np.pi/2
            uav_theta = uav_theta + choose_uav_theta
            uav_theta = uav_theta % (np.pi/2)
        else:
    ...
    def _choose_uav_elevation(self, action, uav_elevation):
        if not self.uav_finish_com_task_flag:
            action_2_temp = np.clip(action[2], 0, 1)
    ...
    def _choose_iotd_power(self, action):
        ...
        # self.pred_channelgain = predict_with_noise(self.model, self.inputs_err, self.env_now, is_noisy_flag=True)
        self.pred_channelgain = self.model.predict([self.inputs,self.env_now], verbose=0) # inputs不归一化
        [gains_min,gains_max] = np.load(r'D:\A_codes\Trans2024\CKM\global_norm_augmented.npy')
        self.pred_channelgain = self.pred_channelgain*(gains_max-gains_min)+gains_min
        if max(self.pred_channelgain) > self.p_min - self.p_max and not self.uav_finish_com_task_flag:
            action_3_temp = 1
        else:
            action_3_temp = 0
    ...
    # 选择覆盖方位得所有用户
    def _inputs(self):
        inputs = np.hstack((self.iotd_pos, np.tile(np.array([self.uav_x_pos, self.uav_y_pos, self.uav_z_pos]),
                                                   (np.array(self.iotd_pos).shape[0], 1))))
        # inputs = (inputs - np.mean(inputs, axis=0)) / (np.std(inputs, axis=0)+0.00000001)
        # inputs /= [1000,1000,250,1000,1000,750]
        inputs_err = np.hstack((self.iotd_pos, np.tile(np.array([self.uav_x_pos, self.uav_y_pos, self.uav_z_pos]),
                                                   (np.array(self.iotd_pos).shape[0], 1))))
        # inputs = (inputs - np.mean(inputs, axis=0)) / (np.std(inputs, axis=0)+0.00000001)
        inputs_err /= [1000,1000,250,1000,1000,750]
        return [inputs,inputs_err]

    def _choose_all_iotd_in_range(self):
        choose_num = []

        self.iotd_received_power_pred_n = self.choose_iotd_power + self.pred_channelgain

        for i in range(self.iotd_num):
            if not self.finish_com_task_flag[i]:
            # 只选择没有完成通信的
                if self.iotd_received_power_pred_n[i] >= self.p_min:
                    choose_num.append(i)
        return choose_num[0:3]

    def _calculate_all_iotd_receive_power(self):
        # self.iotd_receive_power = self.choose_iotd_power + predict_with_noise(self.model, self.inputs, self.env_now,
        #                                                                               is_noisy_flag=False)
        self.iotd_receive_power = self.choose_iotd_power + self.pred_channelgain
        # self.iotd_receive_power = np.array([self.choose_iotd_power] * self.iotd_num) - self.uav_iotd_path_loss_err

    def _mark_uav_iotd_com(self):
        self.is_com = np.zeros(self.iotd_num)
        for i in self.choose_iotd_num_t:
            if self.finish_com_task_flag[i] == 0:
                self.choose_iotd_num[i] = i
    ...
    def _reward_function(self):
        ...
        if self.t < self.t_max:
            ...
            if not self.uav_finish_com_task_flag: # guiding to fly deeper
                reward5_t = real_distance
                reward5 = 0.0005 * reward5_t
            else:
                reward5 = 0
            reward7 = scale7 * sum(self.I)
            reward6 = self.reward6
            # reward = self.scale * (reward1 + reward2 + reward3 + reward4 + 0 + 0 + 0)
            reward = reward2 + 0 + 0.5*reward4 + 0
            ...
            reward7 = 0
            reward = self.scale * (reward1 + reward2 + reward3 + reward4 + 0 + reward6 + reward7)
    ...
    def _save_uav_track_and_com(self):
        # np.savez('pre/pre_noise%d_%d'%(self.ep,self.step_cnt),iotd_received_power_pred_n=self.iotd_received_power_pred_n,choose_iotd_power=self.choose_iotd_power,inputs=self.inputs,payload=self.payload,choose_iotd_num_t=self.choose_iotd_num_t,x=self.uav_vx,y=self.uav_vy,z=self.uav_vz,v=self.uav_v,theta=self.uav_theta,phi=self.uav_elevation,com=self.uav_finish_com_task_flag,uav_com=self.finish_com_task_flag)
        ...
        if np.sum(self.all_ep_endt)/self.t_max == self.uavConfig.train_time:
