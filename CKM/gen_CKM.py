...
from keras.layers import Conv1D, MaxPooling1D, GlobalAveragePooling1D, Lambda, Activation, Multiply
from matplotlib import pyplot as plt
from tensorflow.keras.layers import Dense, Dropout, BatchNormalization, Input, Concatenate, Flatten, Add, Conv2D, \
    GlobalAveragePooling2D, Reshape, Layer
...
def custom_loss(y_true, y_pred):
    ...
    return tf.keras.losses.mean_squared_error(y_true, y_pred_scaled)
def main():
    # 确保每次训练时数据是随机的
    ...
    learning_rate = 0.00001   # 尝试不同的学习率
    ...
    history = model.fit(
        ...
        epochs=3,  # 增加训练轮数
        batch_size=256,  # 使用较小的批处理大小
        validation_split=0.1,
        callbacks=[early_stopping, lr_scheduler, tf_callback]
    )

    model.save('uav_user_channel_model.h5')
