...
def main():
    ...
    file_path = 'user/uav/%s/data/user%d_dis%d/evaluate_log_file' % (date, topConfig.iotd_num, topConfig.total_x)
    file = file_path + '/evaluate_file.txt'
    file= r'D:\A_codes\uav_trajectory_2\uav_trajectory_2\evaluate\user\uav\20220516\01\data\user15_dis1000\evaluate_log_file\evaluate_file.txt'
    ...
    pos[-1][-1]=250
    # iotd_pos = np.zeros((topConfig.iotd_num, 3))
    payload = np.ones((t+1, topConfig.iotd_num))
    v = np.zeros(t + 1)
    fire_pos = np.zeros((topConfig.fire_num, 3))
    power = np.zeros(t+1)
    for k in range(t + 1):
