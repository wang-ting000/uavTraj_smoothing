...
def myplot(pos,pos1, top_config, t, iotd_pos, payload, save_path='trajectory_plot.png'):
    ...
    ax1.plot(pos[:, 0], pos[:, 1], pos[:, 2], color='#1f77b4', marker='^', label='UAV trajectory of PEC-PPO, %ds'%len(pos))

    track1 = np.load(r'D:\A_codes\uavTraj_smoothing\CKM\track2.npy')
    ax1.plot(track1[:, 0], track1[:, 1], track1[:, 2], color='#4b0082', marker='+', label='UAV trajectory of CKM-PPO, %ds'%(len(track1)))
    ...
    los_pos = np.vstack((los_pos,np.array([[0,0,250]])))
    ax1.plot(los_pos[:, 0], los_pos[:, 1], los_pos[:, 2], color='#8b0000', marker='*',
             label='UAV trajectory of LoS-PPO, %ds'%(len(los_pos)))
    import scipy.io
    bcd = scipy.io.loadmat('q3.mat')['q']
    plt.plot(bcd[0, :, 0], bcd[0, :, 1], bcd[0, :, 2], color='#c77608', marker='.',
             label='UAV trajectory of los-BCD, %ds' % (len(bcd[0])))
    ...
    cbar1 = plt.colorbar(p1, pad=-0.1, shrink=0.5,aspect=30)  # 调整色条大小
    ...
    ax2.plot(pos[:, 0], pos[:, 1], pos[:, 2], color='#1f77b4', marker='^', label='UAV trajectory of PEC-PPO, %ds'%(len(pos)))

    ax2.plot(track1[:, 0], track1[:, 1], track1[:, 2], color='#4b0082', marker='+', label='UAV trajectory of CKM-PPO, %ds'%(len(track1)))
    ...
    import scipy.io
    bcd = scipy.io.loadmat('q3.mat')['q']
    plt.plot(bcd[0, :, 0], bcd[0, :, 1], bcd[0, :, 2],color='#c77608',marker='.', label='UAV trajectory of los-BCD, %ds'%(len(bcd[0])))
