...
def main():
    topConfig = uav_gym.envs.uav_env.UavConfig(train_flag=0)
    date1 = r'20240920\00'
    date = '20240920/00/'
    ...
    #model_path = r'D:uav_trajectory\train\user\uav\%s\data\user%d_dis%d\model\uav_ppo2' %(date1,topConfig.iotd_num,topConfig.total_x)
    #model_path = r'C:\Users\Lenovo\PycharmProjects\uav_trajectory\train\user\uav\20230923\01\data\user15_dis1000\model\uav_ppo2.zip'
    model_path = r'D:\A_codes\uavTraj_smoothing\train\user\uav\20240920\00\data\user15_dis1000\model\uav_ppo2.zip'
    model = PPO.load(model_path)
    start_time = time.time()
    evaluate(env=env, model=model, num_steps=5, date=date, topConfig=topConfig)
    end_time = time.time()
